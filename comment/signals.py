...
def send_notificaton(sender, instance, **kwargs):
    ...
    if instance.reply_to is None:
        ...
        if instance.content_type.model == 'article':
            ...
            verb = '{0} 评论了你的《{1}》'.format(instance.user.get_nickname_or_username(), article.title)
        else:
...
def send_email(sender, instance, **kwargs):
    if settings.TURN_ON_EMAIL == True:
        # 发送邮件通知
        ...
        if email != '':
            context = {}
            context['comment_user'] = str(instance.user.get_nickname_or_username())
            context['article_title'] = instance.content_object.title
